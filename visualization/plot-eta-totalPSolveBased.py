...
for case in cases:
    ...
    RK311Data = dataframes[case].loc[["RK311"],["MPIprocs","totalSolveTime"]]
    otherIntegData = dataframes[case].loc[[otherIntegName],["MPIprocs","totalSolveTime"]]
   
    eta[case] =  {}
    eta[case]["MPIprocs"] = otherIntegData.loc[:,["MPIprocs"]].values
    eta[case]["eta"] = (1 - otherIntegData.loc[:,["totalSolveTime"]].values/RK311Data.loc[:,["totalSolveTime"]].values)*100
...
leg_2 = ax.legend(lines[:],labels[:],ncol=1,loc='lower left',bbox_to_anchor=(0,0,1,1),fontsize=10,frameon=False, columnspacing=1.0)
...
ax.set_ylabel(r'% $\eta$ total Pressure Solve'   , fontsize=12)
...
plt.tight_layout()
plt.savefig('./figures/fixed-courant-totalPSolve-eta.pdf',transparent=True)
plt.show()