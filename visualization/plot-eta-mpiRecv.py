...
for case in cases:
    ...
    RK311Data = dataframes[case].loc[["RK311"],["MPIprocs","totalMPIRecv"]]
    otherIntegData = dataframes[case].loc[[otherIntegName],["MPIprocs","totalMPIRecv"]]
   
    eta[case] =  {}
    eta[case]["MPIprocs"] = otherIntegData.loc[:,["MPIprocs"]].values
    eta[case]["eta"] = (1 - otherIntegData.loc[:,["totalMPIRecv"]].values/RK311Data.loc[:,["totalMPIRecv"]].values)*100
...
ax.set_ylabel(r'% $\eta$ total MPI Recv'  , fontsize=12)
...
plt.tight_layout()
plt.savefig('./figures/fixed-courant-mpiRecv-eta.pdf',transparent=True)
plt.show()