...
for case in cases:
    ...
    RK311Data = dataframes[case].loc[["RK311"],["MPIprocs","totalMPISend"]]
    otherIntegData = dataframes[case].loc[[otherIntegName],["MPIprocs","totalMPISend"]]
   
    eta[case] =  {}
    eta[case]["MPIprocs"] = otherIntegData.loc[:,["MPIprocs"]].values
    eta[case]["eta"] = (1 - otherIntegData.loc[:,["totalMPISend"]].values/RK311Data.loc[:,["totalMPISend"]].values)*100
...
ax.set_ylabel(r'% $\eta$ total MPI Send'   , fontsize=12)
...
plt.tight_layout()
plt.savefig('./figures/fixed-courant-mpiSend-eta.pdf',transparent=True)
plt.show()