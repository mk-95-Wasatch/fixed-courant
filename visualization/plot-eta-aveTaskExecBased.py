...
dataType = "aveComponentTimes.csv"
dataframes = {}
for case in cases:
...
for case in cases:
    ...
    RK311Data = dataframes[case].loc[["RK311"],["MPIprocs","aveTaskExec"]]
    otherIntegData = dataframes[case].loc[[otherIntegName],["MPIprocs","aveTaskExec"]]
   
    eta[case] =  {}
    eta[case]["MPIprocs"] = otherIntegData.loc[:,["MPIprocs"]].values
    eta[case]["eta"] = (1 - otherIntegData.loc[:,["aveTaskExec"]].values/RK311Data.loc[:,["aveTaskExec"]].values)*100
...
ax.set_ylabel(r'% $\eta$ aveTaskExec'   , fontsize=12)
...
plt.tight_layout()
plt.savefig('./figures/fixed-courant-aveTaskExec-eta.pdf',transparent=True)
plt.show()